...
class InventarioDao extends DatabaseAccessor<AppDatabase> with _$InventarioDaoMixin {
  ...
  Future<List<InventarioTable>> getInventarioByAlmacen(String almacenId) {
    return (select(inventarios)..where((tbl) => tbl.almacenId.equals(almacenId))).get();
  }

  // Obtener inventario por tienda
  Future<List<InventarioTable>> getInventarioByTienda(String tiendaId) {
    return (select(inventarios)..where((tbl) => tbl.tiendaId.equals(tiendaId))).get();
  }
  ...
  Future<InventarioTable?> getInventarioByProductoAlmacen(
    ...
    return (select(inventarios)
          ..where((tbl) =>
              tbl.productoId.equals(productoId) & tbl.almacenId.equals(almacenId)))
        .getSingleOrNull();
  }
  ...
  Future<List<InventarioTable>> getProductosStockBajo(String almacenId) {
    ...
    return (select(inventarios)
          ..where((tbl) =>
              tbl.almacenId.equals(almacenId) & tbl.cantidadDisponible.isSmallerThanValue(10)))
        .get();
  }

  // Watch inventario (tiempo real)
  Stream<List<InventarioTable>> watchInventarioByAlmacen(String almacenId) {
    return (select(inventarios)..where((tbl) => tbl.almacenId.equals(almacenId))).watch();
  }
  ...
  Future<bool> updateStock(
    ...
    final result = await (update(inventarios)..where((tbl) => tbl.id.equals(inventarioId)))
  ...
  Future<int> insertInventario(InventarioTable inventario) {
    return into(inventarios).insert(InventariosCompanion.insert(
      id: inventario.id,
  ...
  Future<void> incrementarStock(String inventarioId, int cantidad) async {
    final inventario = await (select(inventarios)
          ..where((tbl) => tbl.id.equals(inventarioId)))
        .getSingleOrNull();
